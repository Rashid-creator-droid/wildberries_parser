...
- Логгирование результатов работы парсера в папку [logs](logs).
...
```
https://github.com/Rashid-creator-droid/wildberries_parser.git
```
### Запустить сборку контейнера
```
sudo docker-compose up -d
``` 
Для запуска docker-compose с параметрами добавить в [docker-compose.yml](docker-compose.yml):
```yaml
environment:
  - LIMIT_PAGES=5
  - LIMIT_PER_PAGE=50
...
command: >
      bash -c "python main.py
                ...
                --limit-per-page ${LIMIT_PER_PAGE:-}
                --use-filters ${USE_FILTERS:-false}"
```
## Возможные улучшения проекта.

### Динамическое определение полей таблицы.

  Реализовать механизм формирования полей таблицы на основе subj_name и subj_root_name.
  Поля должны определяться исходя из типа товара, а не быть жёстко заданными.
  Это сделает парсер более гибким и позволит без доработок парсить любые наборы данных и категории товаров.
### Логгирование исключений
    
Добавить логгирование ошибок и исключений.
### Возобновление парсинга с заданной страницы

Добавить возможность указывать страницу, с которой начинается парсинг.
Это позволит продолжать работу после прерывания, используя информацию о уже обработанных страницах, которая сохраняется в [logs](logs).
### Расширение тестового покрытия

Дописать и расширить набор тестов, включая:

- тесты для разных типов товаров и структур данных;

- тесты на обработку ошибок и исключений;
